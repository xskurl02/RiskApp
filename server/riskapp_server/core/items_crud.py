...
def create_item(db: Session, user_id: uuid.UUID, project_id: uuid.UUID, payload, Model):
    ...
    # Treat whitespace-only codes as "missing" so we never store an empty identifier.
    ...
    # Some schemas include optimistic locking fields even for create requests.
    data.pop("base_version", None)
    # Not every DB model necessarily has a "type" column.
