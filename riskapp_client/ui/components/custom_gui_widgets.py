...
from PySide6.QtCore import Qt, QRect, QSize # pylint: disable=no-name-in-module
...
class LoginDialog(QDialog):
    def __init__(self, *, default_url: str = "http://localhost:8000", parent=None) -> None:
    ...
    def values(self) -> Tuple[str, str, str]:
        return (self.url.text().strip(), self.email.text().strip(), self.password.text())
...
class ExcelSelectionDelegate(QStyledItemDelegate):
    ...
    Also renders a 'row number gutter' inside the Title column (col 0).
    """
    ...
    def paint(self, painter, option, index):
        ...
        if index.column() == 0:
            ...
            align = Qt.AlignVCenter | (Qt.AlignHCenter if full_w <= title_rect.width() else Qt.AlignLeft)
...
class RiskForm(QWidget):
    ...
    def __init__(self, on_submit) -> None:
        ...
        # --- Core fields ---
        self.code = QLineEdit()
        self.code.setPlaceholderText("Optional (e.g. R-001 / O-001). Leave empty to auto-generate on server.")
        ...
        if self.owner_user_id.lineEdit():
            self.owner_user_id.lineEdit().setPlaceholderText("Select member or paste UUID (optional)")
        ...
        for w in (self.impact_cost, self.impact_time, self.impact_scope, self.impact_quality):
        ...
        self.identified_at.setPlaceholderText("ISO datetime or date (e.g. 2026-02-19 or 2026-02-19T10:00:00)")
        ...
        self.status_changed_at.setPlaceholderText("ISO datetime (auto when status changes)")
    ...
    def set_members(self, members) -> None:
        ...
        for m in (members or []):
            try:
                uid = getattr(m, "user_id", None) or (m.get("user_id") if isinstance(m, dict) else None)
                email = getattr(m, "email", None) or (m.get("email") if isinstance(m, dict) else None)
    ...
    def set_values(
        ...
        *,
        ...
        widgets = [
            self.code, self.title, self.category,
            self.description, self.threat, self.triggers,
            self.identified_at, self.response_at, self.occurred_at,
        ]
        for w in widgets:
            w.blockSignals(True)
        self.owner_user_id.blockSignals(True)
        ...
        self.impact_cost.setValue(int(impact_cost) if impact_cost is not None else int(impact))
        self.impact_time.setValue(int(impact_time) if impact_time is not None else int(impact))
        self.impact_scope.setValue(int(impact_scope) if impact_scope is not None else int(impact))
        self.impact_quality.setValue(int(impact_quality) if impact_quality is not None else int(impact))
        self._recompute_overall_impact()
