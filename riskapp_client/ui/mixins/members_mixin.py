...
class MembersMixin:
    ...
    def _refresh_members(self) -> None:
        ...
        if offline:
            tab.members_hint.setText("Offline mode: members/roles can be managed only when connected to the server.")
        else:
            tab.members_hint.setText("Project members and roles (admin only for changes).")
            try:
        ...
        for row, m in enumerate(members):
            tab.members_table.setItem(row, 0, self._mk_item(m.email, entity_id=str(m.user_id)))
