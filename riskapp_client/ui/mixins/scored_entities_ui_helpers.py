...
from PySide6.QtWidgets import QLineEdit, QSpinBox, QTableWidget, QTableWidgetItem  # pylint: disable=no-name-in-module

from riskapp_client.services import filters
...
def date_bounds(from_edit: QLineEdit, to_edit: QLineEdit) -> tuple[datetime | None, datetime | None]:
    ...
    *,
    ...
    for row, it in enumerate(items):
        ...
        for col, (val, use_id, center) in enumerate(cols):
            table.setItem(row, col, mk_item(val, entity_id=it.id if use_id else None, align_center=center))
