...
def _opt_str(data: Mapping[str, Any], key: str, default: str | None = None) -> str | None:
    v = data.get(key)
    if v is None:
        return default
    s = str(v).strip()
...
def _opt_int(data: Mapping[str, Any], key: str, default: int | None = None) -> int | None:
    v = data.get(key)
    if v is None:
        return default
