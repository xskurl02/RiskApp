...
class ScoredEntitiesTab(QWidget):
    ...
    def __init__(
        ...
        *,
        ...
        # --- EXCEL-STYLE COLUMN SIZING ---
        hh = self.ui.table.horizontalHeader()
        hh.setSectionResizeMode(QHeaderView.ResizeToContents)
        # 1. Make all columns auto-resize to fit their widest content (header or cell)
        ...
        # 2. Make specifically the "Title" column (index 1) stretch to fill remaining window space
        #hh.setSectionResizeMode(1, QHeaderView.Stretch)

        # --- CONTEXT HELP (HEADER TOOLTIPS) ---
        # Assuming your columns are: 0:Code, 1:Title, 2:Category, 3:Status, 4:Owner, 5:P, 6:I, 7:Score
        # (Wrap in a try-except just in case the UI file hasn't initialized the header items yet)
        # --- CONTEXT HELP (HEADER TOOLTIPS) ---
        if self.ui.table.horizontalHeaderItem(0):
            self.ui.table.horizontalHeaderItem(0).setToolTip("Code: A unique identifier or short reference")
        if self.ui.table.horizontalHeaderItem(1):
            self.ui.table.horizontalHeaderItem(1).setToolTip("Title: The name or brief summary")
        if self.ui.table.horizontalHeaderItem(2):
            self.ui.table.horizontalHeaderItem(2).setToolTip("Category: The classification or grouping")
        if self.ui.table.horizontalHeaderItem(3):
            self.ui.table.horizontalHeaderItem(3).setToolTip("Status: The current lifecycle state")
        if self.ui.table.horizontalHeaderItem(4):
            self.ui.table.horizontalHeaderItem(4).setToolTip("Owner: The team member assigned to manage this")
        if self.ui.table.horizontalHeaderItem(5):
            self.ui.table.horizontalHeaderItem(5).setToolTip("Probability: The likelihood of this occurring (1-5)")
        if self.ui.table.horizontalHeaderItem(6):
            self.ui.table.horizontalHeaderItem(6).setToolTip("Impact: The severity if this occurs (1-5)")
        if self.ui.table.horizontalHeaderItem(7):
            self.ui.table.horizontalHeaderItem(7).setToolTip("Score: Calculated as Probability × Impact (1-25)")
        # --------------------------------------


        # --- CONTEXT HELP (FILTERS & BUTTONS) ---
        ...
        self.ui.filter_from.setToolTip("Filter items identified on or after this date (YYYY-MM-DD)")
        self.ui.filter_to.setToolTip("Filter items identified on or before this date (YYYY-MM-DD)")
        ...
        # --- FORCE 20/80 SPLIT (Editor vs Table) ---
        ...
        # --- DESELECT WHEN CLICKING EMPTY SPACE INSIDE TABLE ---
        ...
        # --- DESELECT WHEN CLICKING EMPTY SPACE AROUND THE TABLE ---
        ...
        # --- FIX 1: THE STARTUP BUG ---
        # Silently set the max score to 25 on startup so the table isn't empty
    ...
    def clear_filters(self) -> None:
        ...
        # --- FIX 2: PREVENT REFRESH STORM ---
        # Block signals so we don't trigger 8 separate database refreshes!
        widgets = [
            self.filter_search, self.filter_min_score, self.filter_max_score,
            self.filter_status, self.filter_category, self.filter_owner,
        ...
        # Now trigger exactly ONE refresh
        self._on_refresh()
